...
public class ServiceExporter implements InitializingBean {
    ...
    private static final Logger LOG = LoggerFactory
	    .getLogger(ServiceExporter.class);

    private static final String SERVER_URL = "http://localhost:8080/ServletBasedJiniServer";
	...
	JoinManager joinManager = new JoinManager(exportedService,
		new Entry[] { new Name(serviceName) }, serviceID,
		discoveryManager, leaseRenewalManager);
	LOG.info(
		"----------------------- Service: {} exported with serviceID: {}",
		serviceName, serviceID);
    }

    private Exporter getExporter() {
	return new BasicJeriExporter(new ServletBasedServerEndpoint(),
		new ServletBasedILFactory(SERVER_URL
			+ REMOTE_METHOD_INVOCATION_URI));
    }
